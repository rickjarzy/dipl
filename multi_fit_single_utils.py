...
def fitq_numpy(lv, pv, A, sq_window):
    """
    using Numpy ndim array
    """

    lv = lv.reshape(lv.shape[0] * lv.shape[1], sq_window, 1)
    pv = pv.reshape(pv.shape[0] * pv.shape[1], 1, sq_window)

    ATP = numpy.multiply(A.T, pv)

    print("Numpy - ATP: ", ATP[0].shape)
    ATPA = numpy.linalg.pinv(numpy.dot(ATP, A))
...
def additional_stat_info_raster_torch(data_block, qual_block, sg_window, device, half_window, center):
    ...
    print("sum : ", torch.sum(noup_tensor, dim=0))
    print("sum shape: ", torch.sum(noup_tensor, dim=0).shape)
    print("\nnoup_l: ", noup_l.shape)
