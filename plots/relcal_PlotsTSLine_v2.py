...
def GetRasterNames(srchPattern,roots):
    ...
    if sat == 'LE7':
        ...
        ODRName = 'ls7_'+pr+'_'+year+doy+'_msp_calibrated_abscal_odr.tif'
    elif sat == 'LT5':
        ...
        ODRName = 'ls5_'+pr+'_'+year+doy+'_msp_calibrated_abscal_odr.tif'
...
# UserParameters
# ==============
UserYear = numpy.arange(2000,2014,1)
...
print "Searching for Epochs - Start :", DOYB, " End :", DOYE
    ...
    for profile in range(0,len(sortedEps),1):
        ...
        for epoch in range(1,satDataSortedEpochs.shape[-1],1):
            ...
            if actualEpochName:     # if actualEpochName is not 'None'
                ...
                #dataFig = plt.step(xAxeArr,arrPD*arrCm,matMarkerColor[epoch-1],markersize=0.5,label=matMarkerLabel[epoch-1])
...
##                if actLineType == 
                ...
                plt.title('Profile of shp Fid: '+str(ft)+'\nTile: '+satDataSortedEpochs[profile,0,1].split("\\")[3]+'Year: 2000-2013\nLSBand: '+str(VisBand)+'\nLineType: '+str(actLineType))
                plt.grid(True)
                plt.hold(True)
    ...
    figName = os.path.join(plotsDir,'S2BIOM_'+satDataSortedEpochs[profile,0,1].split("\\")[-1][:-7]+'_LSBand_'+str(VisBand)+'_Profil_Fid_'+str(ft)+'_Graph.png')
