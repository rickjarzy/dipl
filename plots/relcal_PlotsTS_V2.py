...
def GetRasterNames(srchPattern,roots):
    ...
    if sat == 'LE7':
        ...
        MskName = 'ls5.'+pr+'.'+year+doy+'.msk.tif'
        ...
        if dataProcess[d][l].split("\\")[-1].split('.')[-2] == 'msk':
            ...
            if shp_x >= ras_X_ul or shp_x <= ras_X_lr:
                if shp_y <= ras_Y_ul and shp_y >= ras_Y_lr:
                    ...
                    ras_y_ind_BildMatrix = int(abs((abs(ras_yor)-abs(shp_y))/ras_res))
                    rasData = rasBand.ReadAsArray(ras_x_ind_BildMatrix,ras_y_ind_BildMatrix,1,1)
##                    rasWin = rasBand.ReadAsArray(ras_x_ind_BildMatrix-1,ras_y_ind_BildMatrix-1,3,3)
##                    print rasWin
##                    rasData = numpy.round(numpy.nanmean(rasWin)).astype(numpy.int16)
##                    print rasData
                    if rasData<=0:
                        rasData=0
                    elif rasData>=10000:
...
satDataSortedEpochs = numpy.empty([len(absEpochs),1,6],dtype=object)
...
satDataBase = numpy.zeros([len(absEpochs),6,len(shpPoiKoords)],dtype=int)
...
for shp in range(0,len(shpPoiKoords),1):
    ...
    for e in range(0,len(sortedEps),1):
        ...
        for d in range(0,len(dataProcess),1):
            ...
            for l in range(1,len(dataProcess[d]),1):
                ...
                satDataBase[cou_ep,l,shp]=ReadOutSatData(dataProcess[d][l],VisBand, shp_x, shp_y)
    ...
    absfig = sp.plot(xAxeArr,dataArray[:,1]*dataArray[:,5],'r',markersize=0.5,label='absolut Kal.')
    relfig = sp.plot(xAxeArr,dataArray[:,2]*dataArray[:,5],'g',markersize=0.5,label='relativ Kal.')
    piffig = sp.plot(xAxeArr,dataArray[:,3]*dataArray[:,5],'*b',markersize=5,label='PIF Kal.')
    rbafig = sp.plot(xAxeArr,dataArray[:,4]*dataArray[:,5],'*k',markersize=5,label='RBA Kal.')
    sp.set_ylabel('Landsat Reflectence [%]')
    sp.set_xlabel('Year - DOY []')
    dataArray[:,1]=dataArray[:,1]*dataArray[:,5]
